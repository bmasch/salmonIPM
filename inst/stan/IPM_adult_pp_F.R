...
data {
  ...
  vector<lower=0>[max(year)] R_F_obs;  # observed total wild recruits to the fishery across all pops (assumes no NAs)
  vector<lower=0>[max(year)] C_obs;    # observed total wild catch across all pops (assumes no NAs)
  ...
  for(i in 1:N)
  {
    ...
    else
    {
      ...
      for(j in 2:N_age)  # catch and broodstock removal ???(assumes no take of age 1)???
        S_W[j] = S_W[j]*(1 - F_rate[year[i]])*(1 - B_rate_all[i]);
      S_W_tot[i] = sum(S_W);
